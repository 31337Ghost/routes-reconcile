...
Что делает:
- резолвит `A`-записи доменов через несколько источников: системный резолвер, публичные DNS (`8.8.8.8`, `1.1.1.1`) и Google DoH с EDNS Client Subnet российских сетей — так находятся регионально-специфичные адреса (например, `8.6.112.x`, `8.47.69.x` для `chatgpt.com`), которые локальный резолвер не возвращает;
- разворачивает каждый найденный IP в анонсируемый BGP-префикс через RIPEstat (`8.6.112.6` → `8.6.112.0/24`): DNS ротирует адреса внутри сети, поэтому маршрут на префикс надежнее, чем погоня за отдельными `/32`;
- добавляет отсутствующие маршруты через заданный gateway/interface и приводит к нему уже существующие managed-маршруты;
- удаляет только свои устаревшие маршруты после grace period (по умолчанию 48 часов);
- выполняет одну синхронизацию сразу при старте контейнера, дальше запускается по cron каждые 30 минут через `supercronic`.
...
- `MT_RESOLVERS` — дополнительные DNS-резолверы через запятую (по умолчанию `8.8.8.8,1.1.1.1`); системный резолвер опрашивается всегда, ответы объединяются.
- `MT_ECS_SUBNETS` — клиентские подсети для запросов через Google DoH с EDNS Client Subnet (по умолчанию подсети российских операторов). Пустое значение (`MT_ECS_SUBNETS=`) отключает DoH-запросы.
- `MT_EXPAND_PREFIXES` — `true|false` (по умолчанию `true`): разворачивать IP в анонсируемый BGP-префикс (RIPEstat) и ставить маршрут на сеть вместо `/32`.
- `MT_EXPAND_MIN_PREFIXLEN` — не разворачивать в префиксы крупнее этой длины (по умолчанию `24`): если IP анонсируется, например, как `/13`, маршрут остается `/32`.
...
При старте контейнер сразу выполняет одну синхронизацию (`python /app/main.py`; ошибка первого запуска не роняет контейнер), после чего стартует `supercronic` и читает `crontab`:

```cron
*/30 * * * * python /app/main.py
```

То есть синхронизация выполняется при старте и далее каждые 30 минут.
